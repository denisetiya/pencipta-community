...
export function ResponsiveShell({
  ...
  const renderMobileHeader = showMobileHeader ?? showHeader;
  const renderDesktopHeader = showDesktopHeader ?? showHeader;
    ...
    <FeedProvider>
      {isMobile ? (
        ...
        <div className="relative mx-auto flex h-full min-h-full w-full flex-col bg-white overflow-hidden">
          {renderMobileHeader && <HomeHeader />}
          <main className="relative flex-1 min-h-0 overflow-y-auto flex flex-col">{children}</main>
        ...
        <div className="flex min-h-screen w-full bg-white">
          ...
          <div className="flex flex-1 min-w-0 flex-col bg-white border-r border-zinc-200/80">
            ...
            {/* Main Content Body */}
            <main className="relative flex-1 min-h-0 w-full flex flex-col">{children}</main>

            {/* Universal Bottom Navigation Bar (Visible on mobile screen width) */}
            {showBottomNav && <BottomNav className="md:hidden" />}
          </div>
